...
public class ExchangeMock {
    ...
    public static Map<String, List> mockConfiguration() {
        ...
        List<ExchangeLogStatusTypeType> statusList =
                new ArrayList<>(Arrays.asList(ExchangeLogStatusTypeType.values()));
        ...
        List<TypeRefType> refTypes = new ArrayList<>(Arrays.asList(TypeRefType.values()));
        configuration.put("TYPE", refTypes);

        List<SourceType> sources = new ArrayList<>(Arrays.asList(SourceType.values()));
